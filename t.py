...
def _task_from_taskline(taskline):
    ...
    if '|' in taskline:
        ...
        text = taskline.strip()
        task = { 'id': _hash(text), 'text': text, 'date' : date, 'created' : created }
    return task
...
class TaskDict(object):
    ...
    def add_task(self, text):
        ...
        task_id = _hash(text)
        self.tasks[task_id] = {'id': task_id, 'text': text, 'date' : get_timestamp(), 'created' : get_timestamp()}
    ...
    def print_list(self, kind='tasks', verbose=False, quiet=False, grep='', sort=False):
...
#            print t[1]['date']
        if sort == '':
            ...
            for _, task in sorted(tasks.items(), key=lambda k: k[1][sort]):
                if grep.lower() in task['text'].lower():
                    p = '%s - ' % task[label].ljust(plen) if not quiet else ''
                    print '%s %s' % (p + task[sort], task['text'])
...
def _build_parser():
    ...
    output.add_option("-s", "--sort", dest="sort", default='',
                      help="print sorted by property PROP", metavar="PROP")
    ...
    try:
        if options.finish:
            ...
            td.print_list(verbose=options.verbose, quiet=options.quiet,
                          grep=options.grep, sort=options.sort)
